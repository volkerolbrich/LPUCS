#!/bin/bash
# Linux Power Utils Collection Script (LPUCS) V6.0
# (c)2025 Adam´87 & Emma
...
start_page() {
  ...
  echo -e "${YELLOW}Linux Power Utils Collection Script (LPUCS) V6.0${NC}"
...
check_and_install_packages_de() {
  ...
  # Bei cpupower: Bei apt/apt-get ersetzen wir "cpupower" durch linux-tools
  if ! command -v cpupower &> /dev/null; then 
    if command -v apt-get &> /dev/null || command -v apt &> /dev/null; then 
      missing+=("linux-tools-common" "linux-tools-$(uname -r)")
    else
      missing+=("cpupower")
    fi
  fi
...
check_and_install_packages_en() {
  ...
  if [ ! -f /etc/laptop-mode/laptop-mode.conf ]; then missing+=("laptop-mode-tools"); fi
  if ! command -v auto-cpufreq &> /dev/null; then missing+=("auto-cpufreq"); fi
  # Für cpupower: Bei apt/apt-get ersetzen wir "cpupower" durch linux-tools
  if ! command -v cpupower &> /dev/null; then 
    if command -v apt-get &> /dev/null || command -v apt &> /dev/null; then 
      missing+=("linux-tools-common" "linux-tools-$(uname -r)")
    else
